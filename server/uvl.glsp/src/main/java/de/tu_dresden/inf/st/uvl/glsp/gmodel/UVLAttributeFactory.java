...
public class UVLAttributeFactory extends AbstractSingleGModelFactory<Attribute<?>, GCompartment> {
  ...
  public GCompartment create(Attribute<?> attribute) {
    ...
    if (parentFeature == null) {
      throw new IllegalArgumentException(
          "Attribute does not have a parent feature: " + attribute.getName());
    }
            ...
            .orElseThrow(
                () ->
                    new IllegalStateException(
                        "Parent feature not indexed for attribute: " + attribute.getName()));

    return createAttribute(parentFeatureId, attribute.getName(), attribute);
  }

  protected GCompartment createAttribute(
      final String id, final String attributeName, final Attribute<?> attribute) {
    return createAttribute(GModelUtil.appendAttributeSegment(id, attributeName), attribute, 0);
  }

  protected GCompartment createAttribute(
      final String attributeId, final Attribute<?> attribute, final int indentLevel) {
    ...
    GCompartmentBuilder attributeCompartmentBuilder =
        new GCompartmentBuilder(UVLModelTypes.ATTRIBUTE)
            .id(attributeId)
            .addCssClass("attribute")
            .layout(GConstants.Layout.VBOX)
            .layoutOptions(
                new GLayoutOptions().hAlign(GConstants.HAlign.LEFT).resizeContainer(true))
            .add(
                new GCompartmentBuilder(DefaultTypes.COMPARTMENT)
                    ...
                    .layoutOptions(
                        new GLayoutOptions().hAlign(GConstants.HAlign.LEFT).resizeContainer(true))
                    .add(new GLabelBuilder(DefaultTypes.LABEL).text(indent(indentLevel)).build())
                    .add(
    ...
    GCompartmentBuilder childrenCompartmentBuilder =
        new GCompartmentBuilder(DefaultTypes.COMPARTMENT)
    ...
    for (Map.Entry<String, Attribute<?>> entry : subAttributes.entrySet()) {
      childrenCompartmentBuilder.add(
          createAttribute(
  ...
  protected GCompartment createLeafAttribute(
      final String attributeId, final Attribute<?> attribute, final int indentLevel) {
