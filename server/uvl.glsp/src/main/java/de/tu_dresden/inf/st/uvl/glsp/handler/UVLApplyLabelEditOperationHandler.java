...
public class UVLApplyLabelEditOperationHandler extends GModelApplyLabelEditOperationHandler {
  ...
  protected void executeLabelEdit(final ApplyLabelEditOperation operation) {
    ...
    UVLObject uvlObject =
        modelState
            .getIndex()
            .getUVLObject(parentElement.getId())
            .orElseGet(
                () ->
                    modelState
                        .getIndex()
                        ...
                        .orElseThrow(
                            () ->
                                new IllegalArgumentException(
                                    "No UVL object found for parent element with ID "
                                        + parentElement.getId())));
    ...
    switch (label.getType()) {
      ...
      default ->
          throw new IllegalArgumentException(
              "Label type " + label.getType() + " is not supported for Feature elements.");
    }
  }
  ...
  protected void updateAttributeName(GLabel label, Feature feature, String newName) {
    ...
    if (resolvedAttribute.parentMap().containsKey(newName)
        && !Objects.equals(newName, resolvedAttribute.mapKey())) {
      throw new IllegalArgumentException(
          "An attribute with the name '" + newName + "' already exists at this level.");
    }
    ...
    Attribute<?> newAttribute =
        new Attribute<>(newName, resolvedAttribute.attribute().getValue(), feature);
    replaceMapEntry(
        resolvedAttribute.parentMap(), resolvedAttribute.mapKey(), newName, newAttribute);

    // Constraints can only reference top-level attributes via feature.attribute
    if (resolvedAttribute.path().size() == 1
        && ConstraintUtil.featureAttributeIsInConstraint(
            feature, previousName, modelState.getFeatureModel())) {
      ConstraintUtil.updateFeatureAttributeInConstraints(
          feature, previousName, newName, modelState.getFeatureModel());
    }
  }
  ...
  protected void updateAttributeValue(GLabel label, Feature feature, String newValue) {
    ...
    Attribute<Object> updatedAttribute =
        new Attribute<>(
            previousAttribute.getName(),
            TypeCastingUtil.convertStringToBestType(newValue),
            feature);
    replaceMapEntry(
        resolvedAttribute.parentMap(),
        resolvedAttribute.mapKey(),
        resolvedAttribute.mapKey(),
        updatedAttribute);
  }

  protected void replaceMapEntry(
    ...
    if (newName.isBlank()) {
      ...
      if (!FeatureModelUtil.includesFeatureCardinality(modelState.getFeatureModel())) {
        modelState
            ...
            .remove(LanguageLevel.FEATURE_CARDINALITY);
      }
      return;
    }
