...
public class BPEventFactory extends AbstractSingleGModelFactory<Attribute<?>, GCompartment> {
  ...
  public GCompartment create(Attribute<?> event) {
    ...
    if (parentFeature == null) {
      throw new IllegalArgumentException(
          "B-Event does not have a parent feature: " + event.getName());
    }

    String parentFeatureId =
        modelState
            .getIndex()
            .getIdFor(parentFeature)
            .orElseThrow(
                () ->
                    new IllegalStateException(
                        "Parent feature not indexed for B-Event: " + event.getName()));

    String eventType = BTypeUtil.getBEventType(event);
    String eventId = GModelUtil.appendEventSegment(parentFeatureId, event.getName());

    GCompartmentBuilder eventBuilder =
        new GCompartmentBuilder(eventType)
            .id(eventId)
            ...
            .add(
                new GLabelBuilder(BPModelTypes.EVENT_NAME)
                    .id(eventId + "_name")
                    .text(event.getName())
                    .build());
  ...
  protected GShapePreRenderedElement createIcon(String eventType) {
    return switch (eventType) {
      ...
      case BPModelTypes.BLOCKED_EVENT -> {
        // triangle:
        // https://github.com/microsoft/vscode-codicons/blob/main/src/icons/debug-breakpoint-function.svg
        String d =
            "M12.487 11.249L8.43199 4.24899C8.25199 3.93999 7.74599 3.93999 7.56699 4.24899L3.51199 11.249C3.42199 11.404 3.42199 11.595 3.51199 11.75C3.60199 11.904 3.76699 12 3.94599 12H12.055C12.234 12 12.399 11.904 12.489 11.75C12.578 11.595 12.578 11.404 12.489 11.249H12.487Z";
        yield new GShapePreRenderedElementBuilder(DefaultTypes.SHAPE_PRE_RENDERED)
      ...
      case BPModelTypes.WAITED_FOR_EVENT ->
          new GShapePreRenderedElementBuilder(DefaultTypes.SHAPE_PRE_RENDERED)
  ...
  protected void addPriorityLabel(
      final GCompartmentBuilder eventBuilder, final String eventId, final Attribute<?> event) {
    ...
    eventBuilder
        ...
        .add(
            new GLabelBuilder(BPModelTypes.EVENT_PRIORITY)
