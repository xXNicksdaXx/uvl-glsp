...
import de.tu_dresden.inf.st.uvl.metamodel.model.building.FeatureModelBuilder;
import de.tu_dresden.inf.st.uvl.metamodel.model.building.ParsingUtilities;
...
import de.tu_dresden.inf.st.uvl.metamodel.model.constraint.bp.*;
import de.tu_dresden.inf.st.uvl.metamodel.model.expression.*;
import de.tu_dresden.inf.st.uvl.parser.UVLJavaParser;
import de.tu_dresden.inf.st.uvl.parser.UVLJavaParserBaseListener;
...
public class UVLListener extends UVLJavaParserBaseListener {
    ...
    private final Stack<Constraint> constraintStack = new Stack<>();

    private final Stack<Expression> expressionStack = new Stack<>();

    private final Stack<Map<String, Attribute<?>>> attributeStack = new Stack<>();
    ...
    public void exitSumAggregateFunction(UVLJavaParser.SumAggregateFunctionContext ctx) {
        ...
        if (ctx.reference().size() > 1) {
            ...
            GlobalAttribute attribute = ParsingUtilities.getGlobalAttribute(ctx.reference().getFirst().getText(), fmBuilder.getFeatureModel());
