...
public class ConvertFeatureCardinality implements IConversionStrategy {
    ...
    private void removeFeatureCardinality(Feature feature, FeatureModel featureModel, List<Constraint> constraintsToClone) {
        ...
        for (int i = min; i <= max; i++) {
            ...
            newChild.getAttributes().put("abstract", new Attribute<>("abstract", true, feature));
            ...
            for (int j = 1; j <= i; j++) {
                ...
                subTreeClone.getAttributes().put("abstract", new Attribute<>("abstract", true, subTreeClone));
    ...
    private List<Feature> getFeatureFromSubTree(Group group) {
        List<Feature> features = new LinkedList<>(group.getFeatures());
    ...
    private boolean constraintContains(Constraint constraint, List<Feature> subTreeFeatures) {
        ...
        if (constraint instanceof LiteralConstraint && ((LiteralConstraint) constraint).getReference() instanceof Feature feature) {
        ...
        for (Constraint subPart : subParts) {
            if (subPart instanceof LiteralConstraint && ((LiteralConstraint) subPart).getReference() instanceof Feature feature) {
                if (subTreeFeatures.contains(feature)) {
                    return true;
                }
    ...
    private boolean expressionContains(Expression expression, List<Feature> subTreeFeatures) {
        if (expression instanceof LiteralExpression) {
            Feature feature = ((Attribute<?>) ((LiteralExpression) expression).getContent()).getFeature();
            if (subTreeFeatures.contains(feature)) {
                return true;
            }
        }
