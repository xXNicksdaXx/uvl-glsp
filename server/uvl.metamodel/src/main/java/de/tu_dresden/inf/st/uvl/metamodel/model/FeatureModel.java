...
import de.tu_dresden.inf.st.uvl.metamodel.model.expression.AggregateFunctionExpression;
...
public class FeatureModel {
     ...
     * {@link UVLModelFactory} class how the feature model is assembled
     * there.
     */
    public FeatureModel() {
    }
     ...
     * not contain recursively all imports (including imports of imported feature
     ...
     *
     * @return true if all levels must be explicitly imported, false if not
     */
     ...
     * determines if the used levels are printed in the toString method
     *
    ...
    private String toString(boolean withSubmodels, String currentAlias) {
        ...
        if (rootFeature != null) {
            ...
            result.append(Util.indentEachLine(withSubmodels
                ? getRootFeature().toString(true, currentAlias)
                : getRootFeature().toStringAsRoot(currentAlias)));
            appendAdditionalTopLevelFeatures(result, withSubmodels, currentAlias);
            result.append(Configuration.getNewlineSymbol());
        }
    ...
    /**
     * Extension hook for subclasses that provide additional top-level features.
     */
    protected void appendAdditionalTopLevelFeatures(StringBuilder result, boolean withSubmodels, String currentAlias) {
        // Base feature model has no additional top-level features.
    }
