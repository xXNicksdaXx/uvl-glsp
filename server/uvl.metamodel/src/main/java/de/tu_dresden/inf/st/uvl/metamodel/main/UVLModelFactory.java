package de.tu_dresden.inf.st.uvl.metamodel.main;

import de.tu_dresden.inf.st.uvl.metamodel.conversion.*;
...
import de.tu_dresden.inf.st.uvl.metamodel.model.constraint.bp.AbstractBPEventConstraint;
import de.tu_dresden.inf.st.uvl.metamodel.model.constraint.bp.ConflictingConstraint;
import de.tu_dresden.inf.st.uvl.metamodel.model.expression.*;
...
public class UVLModelFactory {
    ...
    private void convertFeatureModel(FeatureModel rootFeatureModel, FeatureModel featureModel, Set<LanguageLevel> levelsToRemove, Map<LanguageLevel, Class<? extends IConversionStrategy>> conversionStrategies) {
        ...
        while (!levelsToRemoveActually.isEmpty()) {
            LanguageLevel levelToDropNow = levelsToRemoveActually.getFirst();
            levelsToRemoveActually.removeFirst();
            try {
    ...
    private FeatureModel parseFeatureModelWithImports(String text, String rootPath, Map<String, Import> visitedImports, ModelType modelType) {
        ...
        //if featuremodel has no namespace and no root feature getNamespace returns null
    ...
    private void resolveImportPlaceholders(Constraint constraint, FeatureModel featureModel) {
        if (constraint instanceof AndConstraint || constraint instanceof OrConstraint || constraint instanceof NotConstraint || constraint instanceof ImplicationConstraint || constraint instanceof ParenthesisConstraint || constraint instanceof EquivalenceConstraint) {
        ...
        }  else if (constraint instanceof ExpressionConstraint expressionConstraint) {
            ...
            if (literalConstraint.getReference() instanceof ImportedVariablePlaceholder placeholder) {
                literalConstraint.setReference(resolvePlaceholder(placeholder, featureModel));
            }
        } else if (constraint instanceof AbstractBPEventConstraint bpConstraint) {
            if (bpConstraint.getReference() instanceof ImportedVariablePlaceholder placeholder) {
                bpConstraint.setReference(resolvePlaceholder(placeholder, featureModel));
            }
        } else if (constraint instanceof ConflictingConstraint conflictingConstraint) {
            List<VariableReference> refs = conflictingConstraint.getReferenceList();
            for (int i = 0; i < refs.size(); i++) {
                if (refs.get(i) instanceof ImportedVariablePlaceholder placeholder) {
    ...
    private void resolveImportPlaceholders(Expression expression, FeatureModel featureModel) {
        if (expression instanceof BinaryExpression binaryExpression) {
        ...
        } else if (expression instanceof ParenthesisExpression parenthesisExpression) {
            resolveImportPlaceholders(parenthesisExpression.getContent(), featureModel);
        } else if (expression instanceof LengthAggregateFunctionExpression lengthAggregateFunctionExpression) {
            if (lengthAggregateFunctionExpression.getReference() instanceof ImportedVariablePlaceholder placeholder) {
            ...
            if (literalExpression.getContent() instanceof ImportedVariablePlaceholder placeholder) {
    ...
    private boolean validateTypeLevelConstraint(final Constraint constraint) {
        ...
        if (constraint instanceof ExpressionConstraint) {
            ...
            if (!(leftReturnType.equalsIgnoreCase(Constants.TRUE) || rightReturnType.equalsIgnoreCase(Constants.TRUE))) {
                ...
                result = ((ExpressionConstraint) constraint).getLeft().getReturnType().equalsIgnoreCase(((ExpressionConstraint) constraint).getRight().getReturnType());
            }
            if (!result) {
                return false;
            }
