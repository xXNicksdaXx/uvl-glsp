    ...
    private void replaceFeatureInExpression(final Expression expression) {
        if (expression instanceof LiteralExpression) {
            if (((LiteralExpression) expression).getContent() instanceof Attribute<?>) {
                Feature relevantFeature = ((Attribute<?>) ((LiteralExpression) expression).getContent()).getFeature();
                expression.replaceExpressionSubPart(expression, new LiteralExpression(new Attribute<>(Constants.TYPE_LEVEL_VALUE, 0L, relevantFeature))
                );
            }
        }
